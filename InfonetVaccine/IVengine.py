...
class EngineInstance :
    ...
    def scan(self, filename, *callback):
        ...
        while len(file_scan_list):
            try:
                ...
                if os.path.isdir(real_name):
                    ...
                    ret_value['scan_state'] = 0# 악성코드 없음
        ...
        self.__update_process(None, True)  # 최종파일 정리
    ...
    # --------------------------------------------------------------------
    # __update_process(self, file_struct, update_callback_fn,
    ...
    def __update_arc_file_struct(self, p_file_info):
        ...
        if b_update: # 수정된 파일이 존재한다면 재압축 진행
            ...
            can_arc = t[-1].get_can_archive() # 뒤에서 읽은 첫번째 부분
    ...
    # --------------------------------------------------------------------
    # __disinfect_process(self, ret_value, disinfect_callback_fn,
    #                       action_type)
    # 악성코드를 치료한다.
                ...
                for inst in self.kavmain_inst:
                    try :
                        ...
                        if unpack_data :
                            ...
                            # 압축 엔진의 마스터 파일 처리 방법은 getinfo에서 확인 가능
                            try:
                    ...
                    rname_struct = file_struct
                    rname_struct.set_filename(rname)
                    rname_struct.set_can_archive(0)
                return rname_struct
        except IOError:
            pass
        ...
        for inst in self.kavmain_inst:
            ...
            try:
                ...
                if 'engine_type' in ret_getinfo :
                    if ret_getinfo['engine_type'] == 80: # 압축 엔진 Type
    ...
    def set_options(self, options=None) :
        if options :
            ...
            self.options['opt_list'] = options[2]
